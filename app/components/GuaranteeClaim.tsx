...
const STATUS_LABEL: Record<string, string> = {
  ...
  approved_redo: 'Approved — we will arrange a re-do',
  approved_refund: 'Approved — refund from the guarantee reserve',
  rejected: 'Rejected',
};
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.ok && data.claim) {
        ...
        setError(data.error || 'Could not submit the claim. Please try again.');
      }
    } catch {
    ...
    <div className="bg-white rounded-3xl p-5 border border-blue-100/40 mb-5">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="min-w-0">
          <h2 className="text-slate-900 font-black text-sm">Tukola Guarantee</h2>
          <p className="text-slate-500 text-xs leading-snug mt-0.5">
            Something went wrong with this job? File a claim — refunds up to UGX 200,000 from our guarantee reserve.
          </p>
        </div>
      </div>
      ...
      {claim && statusStyle ? (
        <div className="rounded-xl p-3.5 mt-2" style={{ background: statusStyle.bg }}>
          <p className="flex items-center gap-1.5 text-sm font-bold" style={{ color: statusStyle.color }}>
            <statusStyle.Icon size={14} /> {STATUS_LABEL[claim.status]}
          </p>
          ...
          {claim.amountApproved != null && claim.amountApproved > 0 && (
            <p className="text-xs font-bold mt-1.5" style={{ color: statusStyle.color }}>
              Approved amount: UGX {claim.amountApproved.toLocaleString()}
            </p>
          )}
          {claim.resolutionNote && (
            <p className="text-slate-500 text-xs mt-1.5 italic">Note: {claim.resolutionNote}</p>
          )}
        </div>
      ) : !open ? (
        <button
        ...
        <div className="mt-3 space-y-2.5">
          <textarea
            ...
            rows={3}
            placeholder="What went wrong? (e.g. the work was left unfinished, damage was caused…)"
          ...
          <p className="text-slate-400 text-[11px] flex items-center gap-1">
            <AlertCircle size={11} /> Photo upload is coming soon — for now our team may ask you for photos on WhatsApp.
          </p>
          {error && (
