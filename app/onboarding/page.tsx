...
// Copy comes from the i18n dict; only visuals/layout live here.
const SLIDE_META = [
  {
    image: '/images/opt/onboard-1.webp',
    overlay: 'linear-gradient(to top, rgba(10,15,44,0.97) 0%, rgba(10,15,44,0.65) 55%, rgba(10,15,44,0.25) 100%)',
    accent: '#00C8FF',
    tagColor: '#00C8FF',
    keys: { tag: 'ob.s1tag', title: 'ob.s1title', desc: 'ob.s1desc' },
  },
  {
    image: '/images/opt/onboard-2.webp',
    overlay: 'linear-gradient(to top, rgba(26,45,184,0.97) 0%, rgba(26,45,184,0.6) 55%, rgba(0,0,0,0.2) 100%)',
    accent: '#2952E8',
    tagColor: '#fff',
    keys: { tag: 'ob.s2tag', title: 'ob.s2title', desc: 'ob.s2desc' },
  },
  {
    image: '/images/opt/onboard-3.webp',
    overlay: 'linear-gradient(to top, rgba(10,15,44,0.97) 0%, rgba(41,82,232,0.55) 55%, rgba(0,0,0,0.15) 100%)',
    accent: '#00C8FF',
    tagColor: '#00C8FF',
    keys: { tag: 'ob.s3tag', title: 'ob.s3title', desc: 'ob.s3desc' },
  },
];
...
export default function OnboardingPage() {
  ...
  const isLast = current === SLIDE_META.length - 1;
  ...
  // Returning users skip straight to login — must still mark onboarding done,
  // otherwise /login bounces straight back here forever.
  const handleSignIn = () => {
    localStorage.setItem('kola_onboarded', 'true');
    router.push('/login');
  };
    ...
    <div className="min-h-screen relative overflow-hidden bg-[#0A0F2C] flex items-center justify-center">
      ...
      {SLIDE_META.map((s, i) => (
        <div key={i} className="absolute inset-0 transition-opacity duration-700"
          ...
          <Image src={s.image} alt={t(s.keys.tag)} fill style={{ objectFit: 'cover' }} priority={i === 0} />
      ...
      <div className="relative z-10 w-full max-w-lg lg:max-w-xl mx-auto px-5 lg:px-0 lg:mt-16">
        ...
        <div className="lg:glass rounded-3xl lg:p-8">
          ...
          {/* Description */}
          <p className="text-white/65 text-sm lg:text-base leading-relaxed mb-8 max-w-xs">{t(slide.keys.desc)}</p>
          ...
          <button onClick={handleNext}
            ...
            {isLast ? t('ob.getStartedFree') : t('common.continue')}
